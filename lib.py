...
def decode_image(image):
    ...
    # Cast the image tensor into a flast tensor
    image = (tf.cast(image, tf.float32) / 127.5) - 1
    # reshape the tensor into 256 x 256 x 3
...
def downsample(filters, size, apply_instancenorm=True):
    ...
    # group a linear stack of layers into the model
...
def upsample(filters, size, apply_dropout=False):
    ...
    # regularization to reducing overfitting
    if apply_dropout:
